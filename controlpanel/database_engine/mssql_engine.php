...
class MsConnectionFactory extends ConnectionFactory {
    ...
    public function CreateDataReader(IEngConnection $connection, $sql) {
        return new MsDataReader($connection, $sql);
    }
...
class MsCOMConnectionFactory extends ConnectionFactory {
    ...
    public function CreateDataReader(IEngConnection $connection, $sql) {
        return new MsCOMDataReader($connection, $sql);
    }
...
class SqlSrvConnectionFactory extends ConnectionFactory {
    ...
    public function CreateDataReader(IEngConnection $connection, $sql) {
        return new SqlSrvDataReader($connection, $sql);
    }
...
class MsSQLCommandImp extends EngCommandImp {
    ...
    public function GetCastToCharExpression($value, $fieldInfo) {
        if ($this->GetServerVersion()->IsServerVersion(9))
            return sprintf("CAST(%s AS NVARCHAR(max))", $value);
        else
            return sprintf("CAST(%s AS NVARCHAR(4000))", $value);
    }
    ...
    {
        if ($fieldInfo && $fieldInfo->FieldType == ftString) {
            return "N".parent::GetValueAsSQLString($value, $fieldInfo);
        }
        else
            return parent::GetValueAsSQLString($value, $fieldInfo);
    }
...
class MsCOMConnection extends EngConnection {
    ...
    protected function DoConnect() {
        ...
        if ($this->HasConnectionParam('client_encoding') && (strtolower($this->ConnectionParam('client_encoding')) == 'utf-8')) {
            $this->COMConnection = new COM('ADODB.Connection', null, CP_UTF8);
        } else {
...
class MsConnection extends EngConnection {
    ...
    public function ExecScalarSQL($sql) {
        ...
        $queryResult = mssql_fetch_array($queryHandle, MSSQL_NUM);
...
class SqlSrvConnection extends EngConnection {
    ...
    protected function DoConnect() {
       ...
       if ($this->HasConnectionParam('client_encoding') &&
           (
               strtolower($this->ConnectionParam('client_encoding')) == 'utf-8' ||
               strtolower($this->ConnectionParam('client_encoding')) == 'utf8'
           )) {
